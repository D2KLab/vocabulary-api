...
function computeSimilarityScore(l, q, accepted, autocomplete, lowerBound) {
  ...
  let langQuality = accepted ? Lemma.OTHER_LANG_WEIGHT : 1; // untagged values
  if (accepted && curlang) {
  ...
  const startQual = value.startsWith(q) ? 1 : Lemma.UNMATCHING_START_WEIGHT;
  return score * langQuality * startQual;
}

class Lemma {
  static ALT_WEIGHT = 0.7;
  static OTHER_LANG_WEIGHT = 0.6;
  static UNMATCHING_START_WEIGHT = 0.7;
  ...
  pickBestLang(accepted, options = {}, array = null) {
    ...
    const best = acceptLanguage.pick(available, accepted, { loose: true });
  ...
  static setWeights(weights) {
    if (!weights) return;
    Lemma.ALT_WEIGHT = weights.alternate_labels || Lemma.ALT_WEIGHT;
    Lemma.OTHER_LANG_WEIGHT = weights.other_languages || Lemma.OTHER_LANG_WEIGHT
    Lemma.UNMATCHING_START_WEIGHT = weights.unmatching_start || Lemma.UNMATCHING_START_WEIGHT
  }
}
